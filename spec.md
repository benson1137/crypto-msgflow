...
| `okx_oi` | `*/5 * * * *` | oi_funding | mihomo | 数据ts, 15m |
| `rss` | `*/10 * * * *` | events/sightings | mihomo | 心跳 |
| `x_kol` | `*/10 * * * *` | events/sightings | 默认 | 心跳 |
| `bigdata` | `0 1 * * *` | corp_events | mihomo | 心跳 |
| `fomc` | `0 12 * * 1` | macro_calendar | mihomo | 心跳 |
| `listing_alert`(watcher) | 5s 轮询 | Telegram | — | — |

**staleness 两种语义**：
- **数据ts**：节奏源（OKX 每 5m、FRED/TGA 定期发布），最新数据太旧 = 采集器坏了。
- **心跳**：内容驱动源（X/RSS）或 obs_date 结构性滞后源（BLS/BEA 月度数据延迟 4-6 周发布）。活性靠 `collector_runs` 心跳 + 连续 empty 检测，不用数据 ts。
...
| tga | 财政部日频 TGA 余额 |
| okx_oi | BTC-USDT-SWAP, ETH-USDT-SWAP |
